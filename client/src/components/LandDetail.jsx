  ...
  return (
    <div className="animate-fadeIn">
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-bold text-gray-800 mb-2">{land.name}</h1>
        <p className="text-gray-600">Detailed view of your land parcel</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        <div className="lg:col-span-2 bg-white p-6 rounded-2xl shadow-lg">
          <div className="flex justify-between items-start mb-6">
            <div>
              <h2 className="text-2xl font-bold text-gray-800">{land.name}</h2>
              <p className="text-gray-600">{land.location?.address || 'Location not specified'}</p>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div className="bg-green-50 p-4 rounded-lg">
              ...
              <p className="text-2xl font-bold text-gray-800">{land.size} acres</p>
            </div>
            
            <div className="bg-blue-50 p-4 rounded-lg">
              <p className="text-gray-600">Sustainability</p>
              <p className="text-2xl font-bold text-gray-800">{land.sustainabilityScore}%</p>
            </div>
            
            <div className="bg-purple-50 p-4 rounded-lg">
              <p className="text-gray-600">Crops</p>
              <p className="text-2xl font-bold text-gray-800">{land.crops?.length || 0}</p>
            </div>
          ...
          <div className="h-64 bg-gray-100 rounded-lg mb-6 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-gray-600">Land Map Visualization</p>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-lg">
            <h3 className="text-xl font-bold text-gray-800 mb-4">Sustainability Tips</h3>
            <div className="space-y-4">
              <div className="p-4 bg-green-50 rounded-lg border-l-4 border-green-400">
                <h4 className="font-bold text-gray-800">Crop Rotation</h4>
                <p className="text-gray-600 text-sm mt-1">Rotate crops every season to maintain soil health</p>
              </div>
              
              <div className="p-4 bg-blue-50 rounded-lg border-l-4 border-blue-400">
                <h4 className="font-bold text-gray-800">Water Conservation</h4>
                <p className="text-gray-600 text-sm mt-1">Use drip irrigation to reduce water usage by 30%</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {showCropForm && (
        <div className="bg-white p-6 rounded-2xl shadow-lg mb-8 animate-fadeIn">
          <h3 className="text-xl font-bold text-gray-800 mb-4">Add New Crop</h3>
          <form onSubmit={handleCropSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label htmlFor="name" className="block text-gray-700 font-medium mb-2">Crop Name</label>
              <input
                type="text"
                id="name"
                name="name"
            ...
            <div>
              <label htmlFor="plantingDate" className="block text-gray-700 font-medium mb-2">Planting Date</label>
              <input
                type="date"
                id="plantingDate"
                name="plantingDate"
            ...
            <div>
              <label htmlFor="harvestDate" className="block text-gray-700 font-medium mb-2">Harvest Date</label>
              <input
                type="date"
                id="harvestDate"
                name="harvestDate"
            ...
            <div>
              <label htmlFor="yield" className="block text-gray-700 font-medium mb-2">Expected Yield (tons/hectare)</label>
              <input
                type="number"
                id="yield"
                name="yield"
      ...
      {showWaterForm && (
        <div className="bg-white p-6 rounded-2xl shadow-lg mb-8 animate-fadeIn">
          <h3 className="text-xl font-bold text-gray-800 mb-4">Record Water Usage</h3>
          <form onSubmit={handleWaterSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label htmlFor="amount" className="block text-gray-700 font-medium mb-2">Amount (liters)</label>
              <input
                type="number"
                id="amount"
                name="amount"
      ...
      <div className="bg-white p-6 rounded-2xl shadow-lg">
        <h2 className="text-2xl font-bold text-gray-800 mb-4">Crop History</h2>
        {land.crops?.length > 0 ? (
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-gray-200">
                  <th className="pb-3 font-medium text-gray-600">Crop</th>
                  <th className="pb-3 font-medium text-gray-600">Planting Date</th>
                  <th className="pb-3 font-medium text-gray-600">Harvest Date</th>
                  <th className="pb-3 font-medium text-gray-600">Yield</th>
                </tr>
              </thead>
              <tbody>
                {land.crops.map((crop, index) => (
                  <tr key={index} className="border-b border-gray-100 hover:bg-gray-50">
                    <td className="py-4 font-medium text-gray-800">{crop.name}</td>
                    <td className="py-4 text-gray-600">{new Date(crop.plantingDate).toLocaleDateString()}</td>
                    <td className="py-4 text-gray-600">{new Date(crop.harvestDate).toLocaleDateString()}</td>
                    <td className="py-4 text-gray-600">{crop.yield} tons/hectare</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="text-center py-8">
            <p className="text-gray-600">No crop history yet. Add your first crop to get started.</p>
          </div>
        )}
      </div>
    </div>
